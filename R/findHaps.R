...
heatmapByShortestPaths <- function(paired_mapping_rates, gtf, hap_names= c("apd", "cox", "dbb", "mann", "mcf", "pgf", "qbl", "ssto"), penalty=1, sample_name="sample_name"){
  ...
  #########################################################
      ...
      listed_genes <- genes_from_gtf[genes_from_gtf$gene_name %in% gene_list, ]
      listed_genes$chr <- gsub("\\.\\d+", "", listed_genes$chr)
      listed_genes <- listed_genes[listed_genes$chr %in% haps$hap_chrs, ]
            hap_n <- c()
      ...
      listed_genes <- listed_genes[order(listed_genes$hap_n, listed_genes$start), ]
      tmp <- c()
  ...
  #z2 <- unique(z2)
  #z2 <- z2[z2$gene_name %in% dat$gene_name,]
  ...
  selected_genes_by_top1path <- paired_mapping_rates[(paired_mapping_rates$g_h1_h2 %in% toupper(top1_edges$V1) ), ]
